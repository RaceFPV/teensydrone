...
#include <Wire.h> //gyro sensor
#include <Adafruit_GFX.h> //gyro sensor
#include <dht.h> //temp sensor
#include <NewPing.h> //sonar sensors

dht DHT; //enable temp sensor

#define ledblue 23 //multicolor led pin
#define ledgreen 22 //multicolor led pin
#define ledred 21 //multicolor led pin

#define TRIGGER_PIN  12  // sonic sensor pin 1
#define ECHO_PIN     11  // sonic sensor pin 1
#define TRIGGER_PIN2 0 // sonic sensor pin 2
#define ECHO_PIN2 1 // sonic sensor pin 2
#define TRIGGER_PIN3 16 // sonic sensor pin 3
#define ECHO_PIN3 17 // sonic sensor pin 3
#define MAX_DISTANCE 200 // Maximum distance we want to ping for (in centimeters). Maximum sensor distance is rated at 400-500cm.
int MinSafeDistance;
boolean allclearfront; //boolean to see if everything is clear
boolean allclearleft;
boolean allclearright;

#define DHT11_PIN 2 //define temp sensor pin
int tempf; //define fareinheight int

NewPing sonar(TRIGGER_PIN, ECHO_PIN, MAX_DISTANCE); // enable sonic sensor 1
NewPing sonar2(TRIGGER_PIN2, ECHO_PIN2, MAX_DISTANCE); // enable sonic sensor 2
NewPing sonar3(TRIGGER_PIN3, ECHO_PIN3, MAX_DISTANCE); // enable sonic sensor 3
...
// Pin definitions
int intPin = 6;  // This can be changed, 2 and 3 are the Arduinos ext int pins
...
void setup()
{
  Wire.begin();
  Serial.begin(38400);
  MinSafeDistance = 40; //minimum distance from objects to be considered safe
  allclearfront = false;
  allclearleft = false;
  allclearright = false;
...
{  
  Serial.println("");
  //begin temp sensor readings
  int chk = DHT.read11(DHT11_PIN); //read temp sensor pin data
  switch (chk)
  {
    case DHTLIB_OK:  
  ...
  tempf = (DHT.temperature * 2) + 30;
  Serial.println(tempf);
  //end temp sensor readings
  
  
  //start sonar sensor logic
  ...
  Serial.println("cm");
  //end sonar sensor logic
  
  //check if something has come too close to the chopper
  if ((uS / US_ROUNDTRIP_CM == 0) || (uS / US_ROUNDTRIP_CM > MinSafeDistance)) {
    allclearleft = true;
  }
  else {
    allclearleft = false;
  }
  if ((uS2 / US_ROUNDTRIP_CM == 0) || (uS2 / US_ROUNDTRIP_CM > MinSafeDistance)) {
    allclearright = true;
  }
  else {
    allclearright = false;
  }
  if ((uS3 / US_ROUNDTRIP_CM == 0) || (uS3 / US_ROUNDTRIP_CM > MinSafeDistance)) {
    allclearfront = true;
  }
  else {
    allclearfront = false;
  }
  if ((allclearleft == false) || (allclearright == false) || (allclearfront == false)) {
